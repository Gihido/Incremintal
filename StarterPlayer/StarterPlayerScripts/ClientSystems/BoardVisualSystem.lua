...
function BoardVisualSystem.Init(context)
	...
	local coinBoard = createBoard(context, context.BoardPartNames.CoinInfo, "CoinsBoardSurfaceGui", "Coin", "COINS", "Основная валюта")
	local rebirthBoard = createBoard(context, context.BoardPartNames.Rebirth, "RebirthBoardSurfaceGui", "Rebirth", "REBIRTH", "Сброс ради новых зон")
	local woodBoard = createBoard(context, context.BoardPartNames.TreeInfo, "TreeBoardSurfaceGui", "Wood", "WOOD", "Открывается после 2 rebirth")
	local factoryBoard = createBoard(context, context.BoardPartNames.PaperFactory, "PaperFactoryBoardSurfaceGui", "Paper", "PAPER FACTORY", "Топливо превращается в бумагу")
	local boostBoard = createBoard(context, context.BoardPartNames.Boost, "BoostBoardSurfaceGui", "XP", "BOOSTS", "Rebirth / XP / Gamepass статус")
