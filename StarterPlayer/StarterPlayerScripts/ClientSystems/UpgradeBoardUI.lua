...
function UpgradeBoardUI.Init(context)
	...
	for _, boardConfig in ipairs(BOARD_CONFIGS) do
		local partName = context.BoardPartNames[boardConfig.partKey] or boardConfig.partName
		local surface = createSurfaceRoot(context, partName, boardConfig.guiName, boardConfig.title, boardConfig.accent)
		if surface then
			local cards = {}
