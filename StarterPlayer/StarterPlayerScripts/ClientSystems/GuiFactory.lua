...
function GuiFactory.CreateSurfaceGui(playerGui, name, adorneePart, face, pixelsPerStud)
	local surfacePart = resolveBasePart(adorneePart)
	local oldPlayerGui = playerGui and playerGui:FindFirstChild(name)
	if oldPlayerGui then
		oldPlayerGui:Destroy()
	end

	local oldOnPart = surfacePart and surfacePart:FindFirstChild(name)
	if oldOnPart then
		oldOnPart:Destroy()
	end
	...
	surfaceGui.Parent = surfacePart or playerGui
