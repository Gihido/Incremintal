...
local HAY_UPGRADES = PlayerDataSystem.Config.HayUpgrades
...
local function updateHayUpgradeActiveFlags(folder, configTable)
	if not folder then
		return
	end
	...
	for _, config in pairs(configTable) do
		...
		local activeObject = folder:FindFirstChild(config.levelName .. "Active")
...
local function tryBuyUpgrade(player, upgradeKey, configTable, folder, currencyObject, currentRebirthCount)
	...
	if levelObject.Value >= config.maxLevel then
		costObject.Value = 0
		updateHayUpgradeActiveFlags(folder, configTable)
		return "AT_MAX"
	end
...
function HayUpgradeBoard.HandlePurchase(player, upgradeKey, mode)
	...
	if mode == "Max" then
		...
		for _ = 1, 500 do
			local result = tryBuyUpgrade(player, upgradeKey, HAY_UPGRADES, folder, currencyObject, rebirth.Count.Value)
