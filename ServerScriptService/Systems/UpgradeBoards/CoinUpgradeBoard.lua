...
local CoinUpgradeBoard = {}

local COIN_UPGRADES = PlayerDataSystem.Config.CoinUpgrades
...
function CoinUpgradeBoard.HandlePurchase(player, upgradeKey, mode)
	local rebirth = PlayerDataSystem.GetRebirthFolder(player)
	if not rebirth then
		return
	end

	local folder = PlayerDataSystem.GetCoinUpgradesFolder(player)
	local currencyObject = PlayerDataSystem.GetCoinsObject(player)
	if not folder or not currencyObject then
		return
	end

	local notEnoughText = "Не хватает монет"
	...
	if mode == "Max" then
		...
		for _ = 1, 500 do
			local result = tryBuyUpgrade(player, upgradeKey, COIN_UPGRADES, folder, currencyObject, rebirth.Count.Value)
...
function CoinUpgradeBoard.Init()
	if initialized then
		return
	end
	initialized = true
	...
	purchaseUpgradeEvent.OnServerEvent:Connect(function(player, upgradeFamily, upgradeKey, mode)
		...
		CoinUpgradeBoard.HandlePurchase(player, upgradeKey, mode)
	end)
end

return CoinUpgradeBoard
