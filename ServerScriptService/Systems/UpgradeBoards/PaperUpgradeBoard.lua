...
local PAPER_UPGRADES = PlayerDataSystem.Config.PaperUpgrades
...
function PaperUpgradeBoard.HandlePurchase(player, upgradeKey, mode)
	...
	if mode == "Max" then
		...
		for _ = 1, 500 do
			local result = tryBuyUpgrade(player, upgradeKey, PAPER_UPGRADES, folder, currencyObject, rebirth.Count.Value)
