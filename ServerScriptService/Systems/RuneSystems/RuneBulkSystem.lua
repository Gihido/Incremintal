...
function RuneBulkSystem.TryBuy(player, silent)
	local cfg = RUNE_UPGRADES.Bulk
	...
	local currencyObj = PlayerDataSystem.GetPaperCurrencyObject(player)
...
function RuneBulkSystem.TryBuyMax(player)
	for _ = 1, 200 do if not RuneBulkSystem.TryBuy(player, true) then break end end
end

return RuneBulkSystem
