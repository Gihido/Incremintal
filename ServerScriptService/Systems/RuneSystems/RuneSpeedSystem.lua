...
local function fireSimple(player, text)
	...
	if notifyEvent then notifyEvent:FireClient(player, {kind = "simple", text = tostring(text)}) end
end

function RuneSpeedSystem.TryBuy(player, silent)
	local cfg = RUNE_UPGRADES.Speed
	local upgrades = RuneInventorySystem.GetRuneUpgradeFolder(player)
	local rebirth = PlayerDataSystem.GetRebirthFolder(player)
	if not cfg or not upgrades or not rebirth or not rebirth.SecondAreaUnlocked.Value then
	...
	local currencyObj = PlayerDataSystem.GetWoodCurrencyObject(player)
	if not levelObj or not costObj or not currencyObj or levelObj.Value >= cfg.maxLevel then
...
function RuneSpeedSystem.TryBuyMax(player)
	for _ = 1, 200 do if not RuneSpeedSystem.TryBuy(player, true) then break end end
end

return RuneSpeedSystem
